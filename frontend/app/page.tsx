...
import { Briefcase, Code, Bug, Palette, Database, Plus, ArrowRight, FolderKanban, CheckCircle2, Clock } from 'lucide-react';

const AGENTS = [
  { type: 'PM', name: 'PM Agent', icon: Briefcase, color: '#3b82f6', description: 'Project Manager' },
  { type: 'CODING', name: 'Coding Agent', icon: Code, color: '#22c55e', description: 'Full Stack Developer' },
  { type: 'QA', name: 'QA Agent', icon: Bug, color: '#f97316', description: 'Quality Assurance' },
  { type: 'UX', name: 'UX Agent', icon: Palette, color: '#ec4899', description: 'UX Designer' },
  { type: 'DATA', name: 'Data Agent', icon: Database, color: '#06b6d4', description: 'Data Engineer' },
];
...
export default function Dashboard() {
  ...
  const activeTasks = projects.reduce((acc, p) => acc + (p.tasks?.length || 0), 0);
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
        ...
        {/* Agents */}
        <h2 className="text-xl font-semibold mb-4">Our AI Agents</h2>
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4 mb-8">
          {AGENTS.map((agent) => (
            <Link
              key={agent.type}
              href={`/agents?agent=${agent.type}`}
              className="card hover:border-zinc-700 transition-all duration-200 hover:scale-[1.02]"
            >
              <div
